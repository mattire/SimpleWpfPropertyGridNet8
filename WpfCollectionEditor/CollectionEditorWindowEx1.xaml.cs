...
public partial class CollectionEditorWindowEx1 : Window
{
    private readonly IList _list;
    private readonly Type _elementType;
    private readonly IObjectEditorFactory? _objectEditorFactory;
    private readonly Action<object?>? _itemClickedAction;

    /// <summary>
    /// New collection editor that accepts an optional action invoked with the clicked item.
    /// </summary>
    public CollectionEditorWindowEx1(
        IList list,
        Type elementType,
        string title,
        Action<object?>? onItemClicked = null,
        IObjectEditorFactory? objectEditorFactory = null)
    {
        InitializeComponent();
        _list = list;
        _elementType = elementType;
        _objectEditorFactory = objectEditorFactory;
        _itemClickedAction = onItemClicked;
        Title = title;
    ...
    private void RefreshListBox()
    {
        ...
        for (int i = 0; i < _list.Count; i++)
        {
            ...
            {
                var li = new ListBoxItem { Content = item?.ToString() ?? "(null)", Tag = i };
                li.MouseDoubleClick += (s, e) => { _itemClickedAction?.Invoke(item); };
                li.KeyDown += (s, e) => {
                    if (e.Key == System.Windows.Input.Key.Enter) { _itemClickedAction?.Invoke(item); }
                };
                ItemsListBox.Items.Add(li);
            }
        }
    ...
    private void ObjectItemButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        // Invoke provided action with the clicked item (even if null)
        try
        {
            _itemClickedAction?.Invoke(item);
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"CollectionEditorWindowEx item action error: {ex}");
        }
